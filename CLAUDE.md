...
- **hooks/** — Claude Code CLI hook installer for the user-global `~/.claude/settings.json` (same file the App Store opt-in installer targets; `settings.local.json` was the former target but Claude Code only reads it as *project-local* settings when the session cwd is the home directory — the installer/migration scrubs AgentDeck entries out of it), Codex lifecycle hook installer for `~/.codex/config.toml`, and OpenCode observer plugin installer for `~/.config/opencode/plugins/agentdeck.js` (standalone `opencode` sessions POST `opencode_*` lifecycle hooks to the daemon; self-disables in managed PTYs via `AGENTDECK_PORT`)
...
- **Hook format (CRITICAL)**: Claude Code v2.1+ requires 3-level nesting: `{ matcher: "", hooks: [{ type: "command", command: "..." }] }`. Old flat format silently fails. The CLI installer targets the user-global `~/.claude/settings.json` and auto-migrates via `migrateHooksIfNeeded()` (format upgrades + moving hooks stranded in the legacy `settings.local.json`, which Claude Code never reads outside home-directory sessions); the App Store opt-in UI writes the user-selected `~/.claude/settings.json`. Codex uses lifecycle hooks in `~/.codex/config.toml` (`SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`) installed by `installCodexHooksIfNeeded()`. Scripts use bounded `curl` and `|| true` to avoid blocking when bridge is down
